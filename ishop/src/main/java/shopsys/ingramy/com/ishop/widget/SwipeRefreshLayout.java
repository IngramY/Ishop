...
public class SwipeRefreshLayout extends ViewGroup {
    ...
    /**
     * 确保mTarget不为空<br> mTarget一般是可滑动的ScrollView,ListView,RecyclerView等
     */
    ...
    protected void onLayout(boolean changed, int left, int top, int right,
        ...
        LogX.d(LOG_TAG, "debug:onLayout childHeight = " + childHeight);
   ...
   /*     if (mTarget instanceof RecyclerView) {
    ...
    /**
     * 主要判断是否应该拦截子View的事件<br> 如果拦截，则交给自己的OnTouchEvent处理<br> 否者，交给子View处理<br>
     */
    @Override
    ...
    private boolean handlerPushTouchEvent(MotionEvent ev, int action) {
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                mActivePointerId = MotionEventCompat.getPointerId(ev, 0);
                mIsBeingDragged = false;
                LogX.d(LOG_TAG, "debug:onTouchEvent ACTION_DOWN");
                break;
            case MotionEvent.ACTION_MOVE: {
                final int pointerIndex = MotionEventCompat.findPointerIndex(ev,
                        mActivePointerId);
